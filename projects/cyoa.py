...
def outdoor_1() -> None:
    ...
    if search == 1:
        ...
        if search == 2:
            print(f"\nHiding on the trunk of a tree, {name} spots a beautiful butterfly!{BUTTERFLY}")
            points += 15
        else:
            print(f"\nYou find a bumblebee! But {name} gets stung, ouch...{BEE}")
            points += 5
    return outdoor_2()
    ...
    while fish == "yes" and i < 3:
        if i < 2:
            ...
            if i == 2:
                i += 1
                points += fishing(bait)
    print(f"{name} thinks that's enough fishing for today. Onto the next activity!")
    return outdoor_3()


def outdoor_3() -> None:
    """Last outdoor activity. Adds points based on location chosen."""
    print(f"\nYou and {name} decide to go on one more adventure. But {name} can't decide where to go!")
    ...
    if location == 1:
        ...
        if location == 2:
            print(f"{name} loves the forest, and you find some strawberries to snack on. Yum!\n{COW_FACE}{BERRY}")
            points += 15
        else:
            print(f"You and {name} climb all the way to the top. The view is breathtaking!\n{COW_FACE}{MOUNTAIN}")
            points += 5
    return goodnight()
...
def city_1(x: int) -> int:
    ...
    destination: int = int(input(f"But what city should you and {name} visit today?:"
    "\n1. Paris"
    "\n2. New York City"
...
def paris_1(x: int) -> int:
    ...
    print(f"\nBonjour, {player} and {name}, and welcome to Paris!")
    
    CAKE: str = "\U0001F370"
    CROISSANT: str = "\U0001F950"
    BAGUETTE: str = "\U0001F956"
    
    dessert: int = int(input(f"\nFirst, let's head to the bakery! What should you and {name} order?:"
    "\n1. Cake."
    "\n2. A croissant"
    "\n3. A baguette"
    "\nYour order: "))
    ...
    if dessert == 1:
        print(f"\nGreat choice, {player}! Cake is {name}'s favorite!\n{COW_FACE}{CAKE}")
        x += 5
    else:
        if dessert == 2:
            print(f"\nA croissant - nice choice! {name} thinks it's pretty good.\n{COW_FACE}{CROISSANT}")
            x += 3
        else:
            if dessert == 3:
                print(f"\nHmm, {name} isn't a fan of the baguette. More for you, then, {player}!\n{COW_FACE}{BAGUETTE}")
                x+= 1
    return paris_2(x)
...
def paris_2(x: int) -> int:
    ...
    if shopping == 1:
        print(f"\n{name} loves the color, but {name} is a cow, and it's a bit hard to walk...\n{COW_FACE}{SHOE}")
        x += quantity
    else:
        if shopping == 2:
            print(f"\n{name} feels so chic, and now {name} can carry extra snacks!\n{COW_FACE}{BAG}")
            x += (3 * quantity)
        else:
            if shopping == 3:
                print(f"\n{name} feels very fancy in their hat. A perfect fit!\n{COW_FACE}{HAT}")
                x += (5 * quantity)
    return paris_3(x)
...
def paris_3(x: int) -> int:
    ...
    if landmark == 1:
        ...
        if landmark == 2:
            print(f"\n{name} feels so immersed in the city as they walk down the Champs-Elysees!")
            x += 5
        else:
            if landmark ==  3:
                print(f"\nThe view from the Eiffel Tower is incredible! You're so high up, {name} mistakes the people below for ants.")
                x += 5
...
def nyc_1(x: int) -> int:
    ...
    if souvenir == 1:
        print(f"\n{name} buys a mug with the message '{message}' on it! {name} will definitely use this a lot at home.")
        x += 5
    else:
        if souvenir == 2:
            print(f"\n{name} buys a t-shirt with the message '{message}' on it! But {name} is a cow, so it's a bit snug...")
            x += 1
        else:
            if souvenir == 3:
                print(f"\n{name} buys a t-shirt with the message '{message}' on it! {name} doesn't have keys, but still likes it.")
                x += 3 
    return nyc_2(x)
...
def nyc_2(x: int) -> int:
    ...
    print(f"\nNext, you and {name} decide to get some food.")
    ...
    if order == 1:
        ...
        if order == 2:
            print(f"\n{name} thinks the hot dog is pretty good!\n{COW_FACE}{HOT_DOG}")
            x += 5
        else:
            if order == 3:
                print(f"\nGreat choice, {player}. Pizza is {name}'s absolute favorite!\n{COW_FACE}{PIZZA}")
                x += 10
    ...
    if side == 1:
        ...
        if side == 2:
            print(f"{name} thinks the pretzel is delicious! {PRETZEL}")
            x += 5
    return nyc_3(x)
...
def moon_1(x: int) -> int:
    ...
    if scoops == 1:
        ...
        if scoops == 2:
            print(f"You and {name} each get 2 scoops of ice cream {ICE_CREAM} Delicious!")
            x += 3
        else:
            if scoops == 3:
                print(f"You and {name} each get 3 scoops of ice cream {ICE_CREAM} Hurray!")
                x += 5
    return moon_2(x)
...
def moon_2(x: int) -> int:
    ...
    print(f"\nNext, you and {name} go stargazing!")
    ...
    if stargaze == 1:
        ...
        if stargaze == 2:
            print(f"\nLook, {name} can see Saturn from here!\n{SATURN}{TELESCOPE}{COW}")
            x += 5
        else:
            if stargaze == 3:
                print(f"\n{name} sees the Milky Way - as a cow, {name} loves the name.\n{MILKY_WAY}{TELESCOPE}{COW}")
                x += 15
    return moon_3(x)
...
def moon_3(x: int) -> int:
    ...
    print(f"\nJust as you and {name} are about to head home, you spot an alien!")
...
def home_1() -> None:
    ...
    print(f"\nA relaxing day at home sounds perfect to {name}! First, you decide to get a few chores done.")
    ...
    global points
    while len(to_do) > 1:
        chores: int = int(input(f"\n{name} still has a few things on their to-do list:\n{to_do}"
        "\nWhich one would you like to do?: "))
        if chores == 1:
            print(f"\n{name} goes to do their laundry, only cows don't have clothes... {BASKET} Chore #1 done!")
            points += 10
            to_do.remove(laundry)
        else:
            if chores == 2:
                print(f"\n{name} scrubs all the dishes until they sparkle {SOAP} Chore #2 complete!")
                points += 10
                to_do.remove(dishes)
            else:
                if chores == 3:
                    print(f"\n{name} checks their mailbox and finds a letter from their best cow friend {LETTER} Chore #3 finished!")
                    to_do.remove(mail)
                else:
                    if chores == 4:
                        print(f"\n{name} is ready for the next activity in store!.")
                        to_do: list[str] = []
                        home_2()
    
    print(f"\nNice job completing all of {name}'s chores, {player}! Onto the next activity.")
    points += 15
    return home_2()
...
def home_2() -> None:
    ...
    if hobby == 1:
        ...
        if bake == 1:
            ...
            if bake == 2:
                print(f"\n{name} is not the biggest fan of pie, but you make blueberry pie - {name}'s favorite!\n{COW_FACE}{PIE}")
                points += 5
            else:
                if bake == 3:
                    print(f"\n{name} loves any kind of cake, especially cupcakes!\n{COW_FACE}{CUPCAKE}")
                    points += 15
    ...
    if hobby == 2:
        ...
        if flower == 1:
            ...
            if flower == 2:
                print(f"The tulips look so pretty in {name}'s garden {TULIP}")
                points += 10
            else:
                if flower == 3:
                    print(f"{name} thinks the red roses look so beautiful {ROSE}")
                    points += 10

    if hobby == 3:
        points += 20
        print(f"\n{name} paints a beautiful painting of their latest adventures. They are quite the artist.\n{COW_FACE}{PAINT}")
...
def goodnight() -> None:
    ...
    print(f"\nAfter a long day, you and {name} decide it's time for a good night's rest. ")
    BED: str = "\U0001F6CF"
